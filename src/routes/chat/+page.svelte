...
<div class="flex flex-col h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-gray-100">
	<header class="bg-gray-800 text-white p-4 flex justify-between items-center shadow-md">
		<h1 class="text-2xl font-bold text-blue-400">LLAMA Chat 70b</h1>
		<button
			on:click={handleSignOut}
			class="p-2 rounded-full bg-gray-700 text-white hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 transition-all duration-200 transform hover:scale-110"
		>
			<LogOut size={20} />
		</button>
	</header>
		...
		<div class="container mx-auto max-w-4xl">
			{#each messages as message, i (i)}
				...
				>
					...
					>
						<div
							class="w-8 h-8 rounded-full flex items-center justify-center text-white {message.role === 'user'
								? 'bg-blue-500'
								: 'bg-green-500'}"
						>
	...
	<footer class="p-4 bg-gray-800 border-t border-gray-700 shadow-lg">
		<div class="container mx-auto max-w-4xl">
			<form on:submit|preventDefault={sendMessage} class="flex items-center space-x-2">
				...
				<button
					type="submit"
					class="p-2 rounded-md bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50 transition-all duration-200 transform hover:scale-110"
					disabled={isLoading}
				>
					<Send size={20} />
				</button>
